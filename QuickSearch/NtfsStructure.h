...
typedef struct { //sizeof(FILE_RECORD_HEADER)==48
    NTFS_RECORD_HEADER Ntfs;  //16B Ntfs.Type总是'FILE'
	...
	USHORT Align;			// Align to 4 byte boundary
	ULONG RecordNumber;		// Number of this MFT Record
...
typedef struct {   //16B
    ...
    BOOLEAN Nonresident; //本属性不是驻留属性么？
...
typedef struct {
    ATTRIBUTE Attribute; 
    ULONGLONG LowVcn;  //开始的VCN编号
    ULONGLONG HighVcn;  //结束的VCN编号
...
typedef struct { //文件名属性的值区域
    ...
    FILETIME CreationTime; 
    FILETIME ChangeTime; 
    FILETIME LastWriteTime; // 最后一次MFT更新时间
    FILETIME LastAccessTime; 
    ULONGLONG AllocatedSize;
