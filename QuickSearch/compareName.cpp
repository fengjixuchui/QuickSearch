...
bool CompareName(char* chr1,char* chr2)
{
    ...
    int wstrLen = MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)str.c_str(), sLenth,NULL,NULL);
    wchar_t* wcharArr1 = new wchar_t[wstrLen];
    memset(wcharArr1, 0, wstrLen);
    MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)str.c_str(), sLenth, (LPWSTR)wcharArr1, wstrLen);

    std::string str2(chr2);
    sLenth = str2.length() + 1;
    wstrLen = MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)str2.c_str(), sLenth, NULL, NULL);
    wchar_t* wcharArr2 = new wchar_t[wstrLen];
    memset(wcharArr2, 0, wstrLen);
    MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)str2.c_str(), sLenth, (LPWSTR)wcharArr2, wstrLen);
...
{
    int nRet = fileName.find(searchName.c_str());
